...
Persistence failures remain generic to the browser. Vercel function logs include only a safe failure stage and, when Supabase returned a response, its HTTP status; they never include credentials, assessment data, or the Supabase response body. `insert_response status=401` or `403` indicates an invalid/insufficient server credential, while `insert_response status=400` or `404` usually indicates table, column, constraint, or Data API schema mismatch. A stage ending in `_request` indicates a fetch, timeout, network, or rejected-redirect failure. A stage ending in `_representation` indicates an unexpected response body, body-consumption failure, or parse failure.
